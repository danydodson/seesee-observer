export default class PostService {
  ...
  async testingService() {
    this.logger.debug('👉  calling posts test endpoint')
    return { msg: 'posts test route working' }
  }

  async loadPostSlugCtrl(post_slug) {
    this.logger.debug('👉  calling preload post by slug endpoint')
  ...
  async getAllPostsService() {
    this.logger.debug('👉  calling get all posts endpoint')
    const posts = await this.PostModel.find().sort({ date: -1 })
    return { posts }
  }

  async newPostService(user_id, userInput) {
    this.logger.debug('👉  calling new post endpoint')
  ...
  async getPostService(post) {
    this.logger.debug('👉  calling get post endpoint')
  ...
  async updatePostService(post, body) {
    this.logger.debug('👉  calling update post endpoint')
    const {
      mediums,
      title,
      description,
    ...
    if (mediums)
      post.details.mediums =
        mediums.split(', ').map(medium => medium.trim()) || post.details.mediums
    if (title) post.details.title = title || post.details.title
    if (description)
      post.details.description = description || post.details.description
    if (tags)
      post.details.tags =
        tags.split(', ').map(tag => tag.trim()) || post.details.tags
    ...
    if (purchasable)
      post.options.purchasable = purchasable || post.options.purchasable
    // post.updated = Date.now()
    await this.PostModel.findOneAndUpdate(
      { 'links.slug': post.links.slug },
      { $set: post },
      { new: true, upsert: true }
    )
    if (post.options.purchasable) {
      await this.ProfileModel.addListed(post._id)
    }
    if (!post.options.purchasable) {
      await this.ProfileModel.addPosted(post._id)
    }
    // await post.setslug()
    // await post.seturl()
    // await post.save()
    if (post.comments.commentCount > 0) {
      await Comment.updateMany(
        { post: { $in: [post._id] } },
        { $set: { 'links.parent': post.links.slug } },
        { upsert: true }
      )
  ...
  async loadCommentSlugService(post_slug) {
    this.logger.debug('👉  calling preload comment by slug endpoint')
    const comment = await this.CommentModel.findOne({
      'links.slug': comment_slug,
    })
