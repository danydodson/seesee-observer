export default class ProfileService {
  ...
  async testingService() {
    this.logger.debug('👉  calling profiles test endpoint')
    return { msg: 'profiles test route working' }
  }

  async loadUsernamesService(username) {
    this.logger.debug('👉  calling preload username endpoint')
  ...
  async profilesFeedService() {
    this.logger.debug('👉  calling profiles feed endpoint')
    const profiles = await this.ProfileModel.find()
    return { profiles }
  }

  async newProfileService(payload) {
    this.logger.debug('👉  calling new profile endpoint')
  ...
  async getProfileService(foundProfile) {
    this.logger.debug('👉  calling get profile endpoint')
    const profile = foundProfile
    return { profile }
  }

  async updateProfileService(id, body) {
    this.logger.debug('👉  calling update profile endpoint')
    ...
    username
      ? (fresh.details.username = username)
      : (fresh.details.username = old.details.username)
    about
      ? (fresh.details.about = about)
      : (fresh.details.about = old.details.about)
    image ? (fresh.details.image = image) : (fresh.details.image = null)
    fresh.socials = {}
    blog ? (fresh.socials.blog = blog) : (fresh.socials.blog = old.socials.blog)
    instagram
      ? (fresh.socials.instagram = instagram)
      : (fresh.socials.instagram = old.socials.instagram)
    twitter
      ? (fresh.socials.twitter = twitter)
      : (fresh.socials.twitter = old.socials.twitter)
    facebook
      ? (fresh.socials.facebook = facebook)
      : (fresh.socials.facebook = old.socials.facebook)
    youtube
      ? (fresh.socials.youtube = youtube)
      : (fresh.socials.youtube = old.socials.youtube)
    linkedin
      ? (fresh.socials.linkedin = linkedin)
      : (fresh.socials.linkedin = old.socials.linkedin)
    // fresh.vender = {}
  ...
  async addFollowingService(user_id, username) {
    this.logger.debug('👉  calling add following endpoint')
    const isAuthUsersProfile = await this.ProfileModel.findOne({
      user: user_id,
    })
    const noAuthUserProfile = await this.ProfileModel.findOne({
      'details.username': username,
    })
    if (isAuthUsersProfile.isFollowing(noAuthUserProfile._id))
      throw new Error('your already following this user')
  ...
  async addFollowerService(user_id, username) {
    this.logger.debug('👉  calling add follower endpoint')
    const isAuthUsersProfile = await this.ProfileModel.findOne({
      user: user_id,
    })
    const noAuthUserProfile = await this.ProfileModel.findOne({
  ...
  async delFollowingService(user_id, username) {
    this.logger.debug('👉  calling delete following endpoint')
    const isAuthUsersProfile = await this.ProfileModel.findOne({
      user: user_id,
    })
    const noAuthUserProfile = await this.ProfileModel.findOne({
    ...
    if (!isAuthUsersProfile.isFollowing(noAuthUserProfile._id))
      throw new Error('your not following this user')
  ...
  async delFollowerService(user_id, username) {
    this.logger.debug('👉  calling delete follower endpoint')
    const isAuthUsersProfile = await this.ProfileModel.findOne({
      user: user_id,
    })
    const noAuthUserProfile = await this.ProfileModel.findOne({
  ...
  async delProfileService(user_id) {
    this.logger.debug('👉  calling delete profile endpoint')
    const profile = await this.ProfileModel.findOne({ user: user_id })
    if (!profile) throw new Error('cannot remove null profile')
    if (profile.user.toString() !== user_id.toString())
      throw new Error('user not authenticated to do that')
